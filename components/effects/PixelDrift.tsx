...
// Particle-typography effect — ported from originkit.dev's "Pixel Drift"
// (a Framer component; no framer-motion dependency, so this is a near-
// verbatim port with one addition: `fontFamily` is now a prop instead of
// hardcoded, so it can match the site's own display font instead of
// system-ui. The animation/physics internals are left untouched —
// intentionally conservative given how much mouse-repulsion + easing state
// they carry; safer to port faithfully than to hand-simplify code this
// dense. `RenderTarget`/`isStatic` are Framer-canvas concepts that never
// fire outside Framer (kept as inert dead code rather than risk a
// transcription error clearing out branches that gate the whole rAF loop).
...
export function PixelDrift(props: Partial<Props>) {
  const merged: Props = { ...COMPONENT_DEFAULTS, ...props };
  const {
    text,
    colors,
    mode,
  ...
  useEffect(() => {
    ...
    // Canvas's `ctx.font` doesn't resolve CSS custom properties — passing
    // "var(--font-display)" straight through gets silently rejected, the
    // font falls back to whatever the context already had (a tiny default),
    // and sampleText() ends up with almost nothing above the alpha
    // threshold. The container has `fontFamily: resolvedFontFamily` set as
    // a real inline style below, so the browser resolves the var() through
    // the normal CSS cascade; reading it back via getComputedStyle gives
    // the literal, canvas-safe font stack.
    const resolvedFamily = window.getComputedStyle(container).fontFamily || resolvedFontFamily;
    ...
    const sampleText = () => {
      ...
      if (autoFit) {
        effectiveSize = fitFontSize(offCtx, text || '', family, maxW, maxH, Math.max(8, fontSize));
      }

      offCtx.font = `700 ${effectiveSize}px ${family}`;
      ...
      // Original demo exposed this as a 1-50 Framer slider tuned for bold
      // Latin display type, where a 3px stride comfortably covers 15-25px
      // strokes. CJK glyphs pack far more detail into the same em box —
      // strokes run 3-5px even at a large size — so that floor read as a
      // scattered noise cloud instead of legible characters. No upper
      // clamp here; the candidates>30000 downsample below still bounds
      // worst-case particle count regardless of how fine the stride gets.
      const pCount = Math.max(1, particleCount);
